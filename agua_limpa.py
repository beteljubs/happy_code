...
while True:
    sensor = pin2.read_analog()  # lê um valor (analog indica q nao apenas 0 e 1) recebido pelo sensor de luz
...
configurar diferentes leituras:
    quando nada estiver presente (sem o copo) sensor < 230 (ou não, devemos calibrar com a luz do ambiente)
'''
    ...
    if sensor > 230:
        ...
        configurações da fala do alto falante:
            ...
            throat: voz relaxada ou tensa (0 -super tensa- ao 255 -totalmente relaxada-)
        '''

        speech.say("esperando para testar", speed = 120, pitch = 0, throat = 100, mouth =  0)

        stir = 1  # controla quando a colher deve ser mexida

    # caso a agua esteja limpa:
    elif sensor > 100 ad sensor <= 230:
        if stir == 1:
            ...
            if sensor > 100 and sensor <= 230:
                ...
                pin13.write_digital(1)  # acender led azul para indicar q a agua está límpida
        ...
        # caso a agua esteja turva:
            ...
            display.show("TURVA")
            speech.say("turva!", speed = 120, pitch = 0, throat = 100, mouth = 0)
            stir = 1  # caso o sensor leia novamente um valor referente a agua limpa, aciona a colher
